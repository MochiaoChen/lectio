...
2. **Bilibili**：先尝试 `yt-dlp --write-subs --write-auto-subs --sub-langs "zh.*,en.*" --skip-download` 拉字幕；若仍无可用字幕，按"无字幕资源"处理：`yt-dlp -x --audio-format wav` 抽音轨 → `whisper <audio> --model medium --task transcribe --word_timestamps` 生成带时间戳转写。
